/**
 * Dog name cross-pollinator: 175 seeds × 80 passes = 14,000 exactly.
 * Seeds: 161 original dog + 14 cross-type (cat/Sanskrit names that work as dog names)
 * Prefixes: 32 original dog + 47 borrowed from cat/bird/fish/rabbit/hamster/turtle
 * Run: node scripts/expand-dog-names-14k.mjs
 */
...
const DOG_SEEDS = [
  ...
  ["Jasper","male","English","jasper gemstone, treasure","calm, loyal, affectionate","gem,calm,loyal,classic",84,85],
  ["Pepper","female","English","spicy pepper, bold","bold, spirited, sassy","spicy,bold,spirited,classic",83,84],
  ["Harley","male","English","hare's woodland, free","free-spirited, bold, adventurous","woodland,free,bold,biker",82,85],
  ["Bravo","male","Italian","well done, brave","brave, bold, celebratory","brave,bold,italian,classic",80,84],
];

// ── 14 CROSS-TYPE SEEDS (cat/Sanskrit names that work perfectly as dog names) ─

const CROSS_SEEDS = [
  ["Simba",  "male",  "Swahili", "lion, strength",             "regal, adventurous, proud",    "lion,regal,pride,african",       90,89],
  ["Oscar",  "male",  "Irish",   "God's spear, champion",      "wise, proud, independent",     "wise,proud,classic,irish",       88,86],
  ["Felix",  "male",  "Latin",   "happy, fortunate",           "happy, playful, lucky",        "happy,lucky,playful,latin",      87,86],
  ...
  ["Cleo",   "female","Greek",   "glory, pride",               "regal, independent, bold",     "glory,regal,bold,greek",         89,88],
  ["Lily",   "female","English", "pure flower",                "delicate, sweet, graceful",    "flower,pure,sweet,graceful",     90,87],
  ["Violet", "female","Latin",   "purple flower",              "gentle, artistic, sweet",      "flower,purple,artistic,gentle",  85,86],
  ["Iris",   "female","Greek",   "rainbow goddess",            "colorful, graceful, radiant",  "rainbow,goddess,graceful,greek", 84,86],
  ...
  ["Ganga",  "female","Sanskrit","sacred river, pure",         "pure, flowing, divine",        "river,pure,divine,hindu",        80,83],
  ["Gracie", "female","Latin",   "grace, blessing",            "graceful, sweet, gentle",      "grace,gentle,sweet,classic",     86,85],
  ["Truffle","unisex","French",  "rare earthy mushroom",       "rare, earthy, curious",        "luxury,earthy,rare,french",      81,85],
];

const ALL_SEEDS = [...DOG_SEEDS, ...CROSS_SEEDS]; // 175 total

// ── 32 ORIGINAL DOG PREFIXES ──────────────────────────────────────────────────
...
// ── 47 CROSS-TYPE PREFIXES (borrowed from cat/bird/fish/rabbit/hamster/turtle) ─
...
const CROSS_PREFIXES = [
  ...
  // From birds (8)
  { word:"Song",     meaning:"musical, melodious",       keywords:"song,musical,melody",      pop:-5,vibe:+3 },
  ...
  // From fish (11)
  { word:"Marina",   meaning:"of the sea",               keywords:"marina,sea,harbour",       pop:-5,vibe:+2 },
  ...
  { word:"Deep",     meaning:"deep, mysterious",         keywords:"deep,mysterious,vast",     pop:-5,vibe:+3 },
  ...
  // From hamsters (10)
  { word:"Tiny",     meaning:"tiny, miniature",          keywords:"tiny,small,miniature",     pop:-4,vibe:+2 },
  ...
  { word:"Ancient",  meaning:"ancient, enduring",        keywords:"ancient,enduring,timeless",pop:-6,vibe:+3 },
...
const ALL_PREFIXES = [...ORIGINAL_PREFIXES, ...CROSS_PREFIXES]; // 79 total

// ── GENERATION — exact math: 175 seeds × 80 passes = 14,000 ─────────────────

function generateDogNames() {
...
const dogs = generateDogNames();
const slugs = dogs.map(d => d.slug);
const uniqueSlugs = new Set(slugs);
const crossSeeds = dogs.filter(e => CROSS_SEEDS.some(s => s[0] === e.name.split(' ').pop() &&
  !DOG_SEEDS.some(ds => ds[0] === e.name.split(' ').pop() === false)));
const crossPrefixCombos = dogs.filter(e => {
  const parts = e.name.split(' ');
  return parts.length > 1 && CROSS_PREFIXES.some(p => p.word === parts[0]);
});

console.log('─'.repeat(50));
console.log(`Dog seeds       : ${DOG_SEEDS.length}`);
console.log(`Cross-type seeds: ${CROSS_SEEDS.length}`);
console.log(`Total seeds     : ${ALL_SEEDS.length}`);
console.log(`Original pfx    : ${ORIGINAL_PREFIXES.length}`);
...
console.log(`Cross-pfx combos: ${crossPrefixCombos.length}`);
console.log('─'.repeat(50));
console.log('First 10:', dogs.slice(0, 10).map(d => d.name).join(', '));
console.log('Last  10:', dogs.slice(-10).map(d => d.name).join(', '));
console.log('─'.repeat(50));

const outPath = join(__dir, 'dog-names-14000.json');
writeFileSync(outPath, JSON.stringify(dogs, null, 2));
console.log(`Saved → scripts/dog-names-14000.json (${(JSON.stringify(dogs).length / 1024).toFixed(0)} KB)`);
