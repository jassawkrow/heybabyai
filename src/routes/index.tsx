...
function Landing() {
  ...
  const [petPreviewNames, setPetPreviewNames] = useState<Tables<"pet_names">[]>([]);
    ...
    if (mode === "baby") {
      (async () => {
        ...
        if (data && data.length > 0) {
          ...
          setPreviewNames(shuffled.slice(0, 6) as Tables<"names">[]);
        }
      })();
    } else {
      (async () => {
        const { data } = await supabase
          .from("pet_names")
          .select("id, name, meaning_short, origin, pet_type, ai_vibe_score, slug")
          .order("ai_vibe_score", { ascending: false, nullsFirst: false })
          .limit(100);
        if (data && data.length > 0) {
          const shuffled = [...data].sort(() => Math.random() - 0.5);
          setPetPreviewNames(shuffled.slice(0, 6) as Tables<"pet_names">[]);
        }
      })();
    }
  }, [mode]);
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <section className="max-w-5xl mx-auto px-5 sm:px-6 pt-10 sm:pt-16 pb-16 text-center">
        ...
        {/* Primary CTA — destination changes with mode */}
        <div className="mt-7 flex flex-col sm:flex-row gap-3 justify-center">
          {mode === "baby" ? (
            <Link
              to="/swipe"
              className="grad-primary pill text-white font-semibold px-7 py-4 text-sm sm:text-base shadow-xl shadow-purple/20 inline-flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.97] transition"
            >
            ...
            <Link
              ...
              className="grad-primary pill text-white font-semibold px-7 py-4 text-sm sm:text-base shadow-xl shadow-purple/20 inline-flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.97] transition"
            >
              Swipe pet names 🐾 <ArrowRight className="w-4 h-4" />
            </Link>
          )}
        ...
        {/* Search bar — destination changes with mode */}
        <div className="mt-5 max-w-xl mx-auto">
          <div className="glass pill flex items-center gap-3 px-5 py-3.5">
            ...
            {mode === "baby" ? (
              ...
              <Link to="/pets/explore" className="flex-1 text-left text-sm text-ink/40">
                Search 50,000+ pet names… try Luna, Max, Bella
              </Link>
            )}
          </div>
        </div>
      ...
      {/* ── AI Report CTA — hidden in pet mode (baby-specific paywall) ──────── */}
      {mode === "baby" && (
      ...
      <section className="max-w-5xl mx-auto px-5 sm:px-6 pb-20">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          {mode === "baby"
            ...
            : <FeatureCard icon={Search}   grad="grad-3" title="50,000 names, 7 species" body="Dogs, cats, birds, fish, rabbits, hamsters, turtles — filter by type, browse by meaning, find the one." />
          }
        </div>
      </section>

      {/* ── Preview name cards — swaps between baby and pet ──────────────────── */}
